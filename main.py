...
def add_user():
    ...
    if new_account == "" or len(new_account) < 3 or len(new_account) > 20 or " " in new_account:
        user_error = 'That\'s not a valid username'
        errors.append(user_error)
        new_account = ""

    if new_password == "" or len(new_password) < 3 or len(new_password) > 20 or " " in new_password:
    ...
    if len(new_email) > 0 or " " in new_email:
